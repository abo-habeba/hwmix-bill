<template>
  ...
  <v-row dense v-for="(attr, aIndex) in modelValue" :key="aIndex" class="d-flex align-center">
    <v-col cols="5">
      <v-select v-model="attr.attribute_id" :items="attributes" item-title="name" item-value="id" label="اختر الخاصية"
        dense outlined hide-details="auto" @update:modelValue="() => (attr.attribute_value_id = null)" />
    </v-col>
    <v-col cols="5">
      <v-select v-model="attr.attribute_value_id" :items="getAttributeValues(attr.attribute_id)" item-title="name"
        item-value="id" label="اختر القيمة" dense outlined :disabled="!attr.attribute_id" hide-details="auto"
        :style="getAttributeStyle(attr.attribute_id, attr.attribute_value_id)" />
    </v-col>
    <v-col cols="2">
      <v-btn class="ma-0 pa-0" icon="ri-delete-bin-line" color="error" size="x-small" @click="removeAttribute(aIndex)">
      </v-btn>
    </v-col>
  </v-row>
